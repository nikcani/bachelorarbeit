...
Filament basiert auf Livewire\cite{livewire} und daher können innerhalb einer individuellen Komponente auch Funktionen von Livewire genutzt werden.
So gibt es einen eingebauten Polling Mechanismus.
Man fügt lediglich ein Attribut zu einem HTMl Objekt hinzu und Livewire ruft dieses in einem definierbaren Interval erneut vom Server ab.
...
Dadurch kann eine zu nativen Anwendungen vergleichbare UI erstellt werden in welcher der User durch die gesammte Tabelle scrollen kann.

\newlineparagraph{Tabellensortierung}
Nachdem man bei filament eine Spalte als sortierbar definierbar hat, kann man für eine Ansicht ebenfalls eine Standardsortierung angeben.
...
Bei einfachen Komponenten ist es sinnvoll, bei komplexeren Komponenten wie zum Beispiel der Karte, ist es jedoch ein Problem.
...
Durch \enquote{wire:ignore} können allerdings einzelne Bestandteile ausgenommen werden.
...
Die prototypischen Erfahrung entsprechen denen, über die andere Entwickler\cite{reddit-laravel-nova-vs-filament} bereits berichten.
Nova arbeitet impliziter als filament.
Man definiert Felder an einer Stelle, diese gelten dann sowohl für das Formular, als auch für die Tabelle.
Bei filament definiert man ein Feld mindestens an zwei Stellen.

Grundsätzlich erwartet filament, dass für jede eigene Tabelle die Spalten erneut explizit definiert werden.
Dies lässt sich allerdings umgehen, indem man einfach die Tabelle aus der Ressource referenziert.
...
Außerdem ist es durchaus sinnvoll, dass man explizit formuliert, wie die Felder im Formular und wie sie in Tabellen dargestellt werden.
