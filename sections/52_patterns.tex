...
\subsubsection{MVC}
Würde die Sounding Console mit Laravel individuell entwickelt werden, so würde voraussichtlich dieses Pattern zum Einsatz kommen.
Denn Laravel bereitet Models, Views und Controller bereits vor und ermöglicht deren komfortable Nutzung.
Beim Einsatz von Nova und filament fällt jedoch der Controller weitestgehend weg.
...
Das MVC Pattern wird also nur in Teilen umgesetzt.
Dies ist allerdings von Vorteil, da zentrales Feature beider Frameworks die Vereinfachung der Ressourcenverwaltung ist.
Deshalb sollen auch kaum bis keine Controller und klassischen Views implementiert werden, da dies mehr Zeit in Anspruch nehmen würde.

\subsubsection{PAC}
Beide Frameworks setzten wie bereits erwähnt auf das MVC Pattern.
...
Beide Frameworks ermöglichen diese Funktionalität, implementieren sie jedoch nicht selbst.
Nova und filament sind dabei so gebaut, dass im Browser mehrere Tabs oder Fenster der Anwendung geöffnet werden können.
Es gibt webbasierte Anwendungen, die durch mehrere geöffnete Instanzen Probleme mit der Session bekommen.
Die betrachteten Frameworks ermöglichen jedoch, dass der Browser als View Handler zum Einsatz kommt.
Dieses Pattern kann deshalb aus dem Vergleich gestrichen werden, da es zwar zum Einsatz kommt, allerdings in einer anderen Ebene.
...
Semantische Einheiten, bei denen sich dieses Pattern anbieten würde, liegen im aktuellen Projekt nicht vor.

\subsubsection{Layers}
Anhand des Prototyps lässt sich bei beiden Frameworks keine Aufteilung in Schichten erkennen.
Die Anwendung ist an dieser Stelle sehr überschaubar gehalten und eher auf einer Abstraktionsebene angesiedelt.
...
Beide Frameworks verwenden das Pattern, auch wenn sie es jeweils anders benennen.
Dabei ist die Implementierung in beiden Fällen nicht sehr umfangreich und noch ausbaufähig.
...
\subsubsection{Fazit}
Für die vollständige Anwendung eines Patterns gibt es zwei Punkte, für die teilweise Anwendung einen Punkt.
